...
def pre_processing(dict_images):
    ...
    # OARs
    list_OAR_names = ['BLADDER','RECTUM','LT FEMORAL HEAD','RT FEMORAL HEAD']
    ...
    CT=np.clip(CT,a_min=0,a_max=1500)
    CT = CT.astype(np.float32) / 1000.

    # Dose image
    dose = dict_images['dose']/46
...
def train_transform(list_images):
    ...
    list_images = random_translate(list_images,roi_mask=list_images[2][ :,:, :, :], p=0.8,max_shift=20, list_pad_value=[0,0, 0, 0])
...
class MyDataset(data.Dataset):
    def __init__(self, num_samples_per_epoch, phase):
        ...
        self.list_case_id = {'train': [ 'C:\Himani Data\Cx Patient Data\Patient\C3D_results\pt_' + str(i) for i in range(1,67)],'val': ['C:\Himani Data\Cx Patient Data\Patient\C3D_results\pt_' + str(i) for i in range(68,71)]}[phase]
#{'train': ['../../Data/OpenKBP_C3D/pt_' + str(i) for i in range(1, 201)],'val': ['../../Data/OpenKBP_C3D/pt_' + str(i) for i in range(201, 241)]}[phase]

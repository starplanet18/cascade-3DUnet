...
def read_data(patient_dir):
    ...
    list_structures = ['CT',
                      'PTV FINAL',
                       'possible_dose_mask',
                       'BLADDER','RECTUM','LT FEMORAL HEAD','RT FEMORAL HEAD',
                       'dose']
...
def pre_processing(dict_images):
    # PTVs
    PTVs = dict_images['PTV FINAL']


    # OARs
    list_OAR_names = ['possible_dose_mask','BLADDER','RECTUM','LT FEMORAL HEAD','RT FEMORAL HEAD']
...
if __name__ == "__main__":
    if not os.path.exists('C:\Himani Data\Cx Patient Data\Patient\C3D_results'):
        raise Exception('OpenKBP_C3D should be prepared before testing, please run prepare_OpenKBP_C3D.py')

    parser = argparse.ArgumentParser()
    parser.add_argument('--GPU_id', type=int, default=0,
                        help='GPU id used for testing (default: 0)')
    parser.add_argument('--model_path', type=str, default=r'C:\Himani Data\Cx Patient Data\RTDosePrediction-main\RTDosePrediction-main\RTDosePrediction\PretrainedModels')
    ...
    trainer.setting.output_dir = r'C:\Himani Data\Cx Patient Data\RTDosePrediction-main\RTDosePrediction-main\RTDosePrediction\Output'
    ...
    # Start inference
    print('\n\n# Start inference !')
    list_patient_dirs = [r'C:\Himani Data\Cx Patient Data\Patient\C3D_results\pt_' + str(i) for i in range(241, 341)]
    ...
    Dose_score, DVH_score = get_Dose_score_and_DVH_score(prediction_dir=trainer.setting.output_dir + '/Prediction',
                                                         gt_dir=r'C:\Himani Data\Cx Patient Data\Patient\C3D_results')
